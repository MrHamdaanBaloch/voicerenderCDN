...
class VoiceAIAgent(Consumer):
    ...
    async def handle_conversation(self, call: Call):
        ...
        try:
            # Start with a prompt to welcome the user and get their first response.
            prompt_result = await self.play_tts_response(call, "Hello! Thank you for calling. How can I help you today?", use_groq_pipeline=False)
            
            while call.active:
                # If the last prompt didn't capture speech (i.e., no barge-in), we need to listen now.
                if not prompt_result or not prompt_result.successful or not prompt_result.result:
                    ...
                    recording_url = record_action.url
                else:
                    # The user barged-in, so the prompt already has the recording.
                    recording_url = prompt_result.url

                # We have a recording URL, send it to Celery.
                task = celery_app.send_task("get_llm_response_task", args=[call.id, recording_url])
                llm_response_text = task.get(timeout=15)

                if not llm_response_text:
                    logger.error(f"[{call.id}] Worker failed to produce LLM text.")
                    # We'll loop again and listen for the user.
                    prompt_result = None
                    continue
                
                # Play the LLM response and simultaneously listen for the next barge-in.
                prompt_result = await self.play_tts_response(call, llm_response_text, use_groq_pipeline=True)
        except Exception as e:
