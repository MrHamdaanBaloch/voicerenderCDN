...
class VoiceAIAgent(Consumer):
    ...
    async def handle_conversation(self, call: Call):
        ...
        try:
            ...
            recording_to_process = await self.play_tts_response(call, welcome_audio_url, use_groq_pipeline=True)
            ...
            while call.active:
                try:
                    ...
                    if recording_duration < MIN_RECORDING_DURATION_S:
                        logger.warning(f"[{call.id}] Discarding recording of {recording_duration:.2f}s, likely noise.")
                        recording_to_process = None
                        continue
                    ...
                    if task_result is None:
                        logger.warning(f"[{call.id}] Worker returned no result. Looping.")
                        recording_to_process = None
                        continue
            ...
            while True:
                ...
                if record_waiter in done:
                    ...
                    if duration >= MIN_RECORDING_DURATION_S:
                        ...
                        logger.warning(f"[{call.id}] Noise detected (duration: {duration:.2f}s). Ignoring.")
                        continue
        except Exception as e:
